...
rows += `<tr class="sx-data-row" onclick="APP.openDetailS('sx',${_si})"> <td class="sx-date">${day}</td> <td class="sx-area">${r.area}</td> <td class="sx-name">${r.name}</td> <td><span class="badge badge-${r.type}">${r.type||'-'}</span></td> <td class="sx-opname">${r.opName}</td> <td class="sx-loc" title="${r.location}">${r.location}</td> <td class="sx-implant">${r.implant}</td> <td class="sx-note">${r.note}</td> </tr>`;
});
el.innerHTML = rows;
...
const _si=APP._storeRow(r);
rows += `<tr class="sx-data-row" onclick="APP.openDetailS('track',${_si})"> <td class="sx-date">${r.date.substring(5)}</td> <td class="sx-area">${r.area}</td> <td class="sx-name">${r.name}</td> <td><span class="badge badge-${r.type}">${r.type||'-'}</span></td> <td class="sx-opname">${r.opName}</td> <td class="sx-loc" title="${r.location}">${r.location}</td> <td class="sx-implant">${r.implant}</td> <td class="sx-note">${r.note}</td> </tr>`;
});
el.innerHTML = rows;
} catch(e) { el.innerHTML = `<tr><td colspan="8">${this.err(e)}</td></tr>`; }
},
...
html += `<div class="list-row${isNew?' row-new':''}" onclick="APP.openDetailS('mat',${_si})"> ${isNew?'<span class="new-dot"></span>':'<span class="dot-ph"></span>'} <span class="col-brand">${r.brand}</span> <span class="col-product">${r.product}${sub}</span> <span class="col-qty">${r.qty}</span> <span class="col-price">${cleanP?'$'+cleanP.toLocaleString():''}</span> ${isDone ? '<span class="done-ph"></span>' : `<button class="done-btn" onclick="event.stopPropagation();APP.markDone(${r._row})" title="標記完成">☑</button>`} </div>`;
});
});
el.innerHTML = html;
...
const _si=APP._storeRow(r);
html += `<div class="list-row" onclick="APP.openDetailS('selfpay',${_si})"> <span class="col-brand">${r.brand}</span> <span class="col-product">${r.product}</span> <button class="add-center-btn" onclick="event.stopPropagation();APP.qAddMat('${r.brand.replace(/'/g,"\\'")}','${r.product.replace(/'/g,"\\'")}','${cleanP}')" title="新增到骨材記錄">＋</button> <span class="col-price">${cleanP?'$'+Number(cleanP).toLocaleString():'-'}</span> <span class="col-hosp">${r.hospital||''}</span> </div>`;
});
});
el.innerHTML = html;
} catch(e) { el.innerHTML = this.err(e); }
...
const cleanP = parseFloat(String(r.price||0).replace(/,/g,’’))||0;
const _si=APP._storeRow(r);
html += `<div class="list-row" onclick="APP.openDetailS('opcode',${_si})"> <span class="col-code">${r.code}</span> <span class="col-product" title="${r.name}">${r.name}</span> <button class="add-center-btn" onclick="event.stopPropagation();APP.qAddCode('${r.name.replace(/'/g,"\\'")}','${r.code}','${r.price}','${r.area}')" title="新增到代碼紀錄">＋</button> <span class="col-price">${cleanP?'$'+cleanP.toLocaleString():''}</span> </div>`;
});
});
el.innerHTML = html;
} catch(e) { el.innerHTML = this.err(e); }
...
const cleanP=parseFloat(String(r.price||0).replace(/,/g,’’))||0;
const _si=APP._storeRow(r);
html += `<div class="list-row${isNew?' row-new':''}" onclick="APP.openDetailS('coderec',${_si})"> ${isNew?'<span class="new-dot"></span>':'<span class="dot-ph"></span>'} <span class="col-product" title="${r.name}">${r.name}</span> <span class="col-code">${r.code}</span> <span class="col-price">${cleanP?'$'+cleanP.toLocaleString():''}</span> <span class="col-qty">${r.qty}</span> <span class="col-area">${r.area}</span> </div>`;
});
});
el.innerHTML = html;
} catch(e) { el.innerHTML = this.err(e); }
...
const isTotal = li===0;
return `<tr class="${isTotal?'est-total-row':''}"> <td>${lb.label}</td> ${sorted.map(r=>{ const v=r[lb.key]; return `<td>${v&&v!==‘0’?Number(String(v).replace(/,/g,’’)).toLocaleString():’-’}</td>`; }).join('')} </tr>`;
}).join(’’);
